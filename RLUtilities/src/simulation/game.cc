...
void Game::read_flatbuffer_packet(
  ...
  if (num_boost_pads != num_game_boosts) {
    ...
    for (int i = 0; i < num_boost_pads; i++) {
      ...
      if (gameTickPacket->ball()->latestTouch() != nullptr){
          latest_touch_team = gameTickPacket->ball()->latestTouch()->team();
          latest_touch_time = gameTickPacket->ball()->latestTouch()->gameSeconds();
          latest_touch_player = gameTickPacket->ball()->latestTouch()->playerName()->str();
      } else latest_touch_time = 0;
