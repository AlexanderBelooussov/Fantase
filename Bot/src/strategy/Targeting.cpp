...
vec2 Targeting::determineShotDirection(vec3 ball_position) {
    ...
    vec2 result;
    float lowest_score = 999;
    for (vec3 shot : shots){
        float score = 0;
        //std::cout << opponent_indices.size() << std::endl;
        for (int i = 0; i < opponent_indices.size(); i++){
            vec2 intersect = getClosestIntersect(ball_position, shot, i);
            //std::cout << intersect << std::endl;
            if (norm(intersect) > 0){
                float distance_to_opponent = distance(flatten(info.game.cars[opponent_indices[i]].position), intersect);
                float distance_to_ball = distance(ball_position, intersect);
                if (distance_to_ball > distance_to_opponent){
                    score +=  1 + ((distance_to_ball-distance_to_opponent) / distance_to_ball);
                }
                else{
                    score +=  1 - ((distance_to_opponent-distance_to_ball) / distance_to_opponent);
                }
            }
        }
        //enemy net
        ...
        if (postCorrection(ball_position, left_post, right_post)){
            vec2 intersect_with_goal = getIntersection(vec2{-893, 0} + vec2{info.theirGoal}, vec2{1, 0}, ball_position, shot);
        ...
        //our own net
        vec2 intersect_with_my_goal = getIntersection(vec2{-893, 0} + vec2{info.myGoal}, vec2{1, 0}, ball_position, shot);
        if (norm(intersect_with_my_goal) > 0){
            if (abs(intersect_with_my_goal[0]) < 780 ) score -= 2;
        }
        //more center usually more better
